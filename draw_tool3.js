...
const state = {
  ...
  selectedPenPoints: new Set(),  // v9.11: 박스 선택된 펜점 인덱스
  dragPointsMulti: null,         // v9.11: 다중 점 그룹 이동 상태
  parts: [],
  selectedPartId: null,
  history: [],
  historyIdx: -1,
  ...
  gridSnap: true,   // v9.10: 항상 ON (격자 0.1mm 고정)
  objSnap: true,    // v8.11: 객체 스냅 (점/끝점/중점/중심/사분점/수평수직 가이드)
  _lastSnapKind: null,  // 마지막 스냅 종류 — 시각 피드백용
  gridSize: 0.1,    // v9.10: 0.1mm 고정
  angleSnapDeg: 15, // v9.10: 각도 정렬 단위(도) — 15° 기본 (선 그릴 때/점 찍을 때 약한 자석)
  ...
  moveSnap: 1,   // v9.15: 기본 이동단위 1mm (마우스 드래그/키보드 방향키)
  rotSnap: 22.5, // v9.15: 기본 회전단위 22.5° (마우스 회전 핸들)
...
function snapPoint(p){
  ...
  if(state.penCur >= 0 && state.penPoints[state.penCur]){
    ...
    // v9.10: 각도 정렬 자석 (기본 15° 단위) — ref→p 각도가 15°의 배수에 가까우면 그 각도선 위로 당김
    if(state.angleSnapDeg && state.angleSnapDeg > 0){
      const dxr = p.x - ref.x, dyr = p.y - ref.y;
      const dist = Math.hypot(dxr, dyr);
      if(dist > tol){
        const ang = Math.atan2(dyr, dxr);
        const stepRad = state.angleSnapDeg * Math.PI / 180;
        const snapAng = Math.round(ang / stepRad) * stepRad;
        // 각도 편차가 약 7° 이내면 자석 동작 (약하게 멈춤)
        let dA = Math.abs(ang - snapAng);
        if(dA > Math.PI) dA = 2*Math.PI - dA;
        if(dA < (7 * Math.PI/180)){
          const sx = ref.x + Math.cos(snapAng) * dist;
          const sy = ref.y + Math.sin(snapAng) * dist;
          const degTxt = Math.round(snapAng * 180/Math.PI);
          consider(sx, sy, '∠' + ((degTxt%360+360)%360) + '°');
        }
      }
    }
    // 수평선
...
function drawPenLabels(){
  ...
  state.penPoints.forEach((p, i) => {
    ...
    const isSel = state.selectedPenPoints && state.selectedPenPoints.has(i);
    // 점 마커 (노란 원, 선택 시 보라 테두리 강조)
    skCtx.fillStyle = isSel ? '#c084fc' : '#f0c040';
    skCtx.strokeStyle = isSel ? '#7c3aed' : '#000';
    skCtx.lineWidth = isSel ? 2 : 1;
    skCtx.beginPath();
    skCtx.arc(sp.x, sp.y, isSel ? 5 : 3.5, 0, Math.PI*2);
    skCtx.fill();
    skCtx.stroke();
    // 번호 + (옵션) 좌표 텍스트
    let label = 'P' + i;
...
// v9.23: 좌표점 등록 헬퍼 — 같은 위치 점이 없으면 penPoints 에 추가하고 인덱스 반환
function sk3EnsurePoint(x, y){
  if(!Array.isArray(state.penPoints)) state.penPoints = [];
  const tol = 0.01;
  let idx = state.penPoints.findIndex(p => Math.abs(p.x-x)<tol && Math.abs(p.y-y)<tol);
  if(idx < 0){ state.penPoints.push({x, y}); idx = state.penPoints.length - 1; }
  return idx;
}
...
skCanvas.addEventListener('mousedown', (e)=>{
  ...
  if(e.button === 0){
    ...
    if(hp >= 0){
      ...
      // v9.11: 박스로 다중 선택된 점을 클릭하면 그 점들 전체를 함께 이동
      if(!e.shiftKey && state.selectedPenPoints && state.selectedPenPoints.size > 1 && state.selectedPenPoints.has(hp)){
        const idxs = [...state.selectedPenPoints];
        state.dragPointsMulti = {
          idxs,
          origs: idxs.map(i => ({x:state.penPoints[i].x, y:state.penPoints[i].y})),
          startWp: wp, moved:false,
          lineRefs: (function(){
            const tol=0.05; const refs=[];
            state.shapes.forEach((s,si)=>{ if(s.type!=='line')return;
              idxs.forEach(pi=>{ const p=state.penPoints[pi];
                if(Math.abs(s.x1-p.x)<tol&&Math.abs(s.y1-p.y)<tol) refs.push({si,end:1,orig:{x:s.x1,y:s.y1}});
                if(Math.abs(s.x2-p.x)<tol&&Math.abs(s.y2-p.y)<tol) refs.push({si,end:2,orig:{x:s.x2,y:s.y2}});
              });
            });
            return refs;
          })()
    ...
    // v9.11: 선택 도구로 빈 곳 클릭(점/도형 모두 미히트) → 다중 점 선택 해제
    if(state.tool === 'select' && !e.shiftKey && state.selectedPenPoints && state.selectedPenPoints.size > 0){
      const hp2 = sk3FindNearestPenPoint(wp);
      if(hp2 < 0){ state.selectedPenPoints.clear(); redrawSketch(); }
    }
  }
  ...
  if(state.tool === 'line'){
    if(!state.drawing){
      ...
      // v9.23: 선 양 끝에 좌표점 생성 (점에서 시작/연결 가능)
      sk3EnsurePoint(state.drawing.start.x, state.drawing.start.y);
      state.penCur = sk3EnsurePoint(wp.x, wp.y);
      state.drawing = {type:'line', start: wp, current: wp};
      updateInfo();
    }
...
skCanvas.addEventListener('mousemove', (e)=>{
  ...
  // v9.11: 다중 선택 점 그룹 이동
  if(state.dragPointsMulti){
    const dm = state.dragPointsMulti;
    let ddx = wp.x - dm.startWp.x, ddy = wp.y - dm.startWp.y;
    if(state.gridSnap){ const g=state.gridSize; ddx=Math.round(ddx/g)*g; ddy=Math.round(ddy/g)*g; }
    if(!dm.moved && Math.hypot(ddx,ddy) < 0.05) return;
    dm.moved = true;
    dm.idxs.forEach((pi,k)=>{ const o=dm.origs[k]; state.penPoints[pi].x=o.x+ddx; state.penPoints[pi].y=o.y+ddy; });
    dm.lineRefs.forEach(r=>{ const s=state.shapes[r.si]; if(!s)return; if(r.end===1){s.x1=r.orig.x+ddx;s.y1=r.orig.y+ddy;} else {s.x2=r.orig.x+ddx;s.y2=r.orig.y+ddy;} });
    redrawSketch(); updateInfo();
    return;
  }
...
skCanvas.addEventListener('mouseup', (e)=>{
  ...
  if(state.dragPointsMulti){
    const dm = state.dragPointsMulti;
    if(dm.moved){ pushHistory(); setStat('✓ 점 ' + dm.idxs.length + '개 이동'); }
    state.dragPointsMulti = null;
...
function sk3FinishBoxSelect(box){
  if(!box) return;
  if(!box.addToSel){ state.selectedShapes.clear(); state.selectedPenPoints.clear(); }
  ...
  const ib = (x,y) => x >= minX && x <= maxX && y >= minY && y <= maxY;
  // v9.11: 펜점도 박스 안이면 선택
  (state.penPoints||[]).forEach((p,i)=>{ if(ib(p.x,p.y)) state.selectedPenPoints.add(i); });
  // v9.11: 도형의 일부라도 박스에 걸치면 선택 (끝점 포함 OR 선분이 박스와 교차)
  state.shapes.forEach((s, i) => {
    let hit = false;
    if(s.type === 'line'){
      hit = ib(s.x1,s.y1) || ib(s.x2,s.y2) || segIntersectsBox(s, minX,minY,maxX,maxY);
    } else if(s.type === 'rect'){
      // 사각형 4변 중 하나라도 박스와 겹치거나, 사각형이 박스를 포함/교차
      const r1x=Math.min(s.x1,s.x2), r2x=Math.max(s.x1,s.x2);
      const r1y=Math.min(s.y1,s.y2), r2y=Math.max(s.y1,s.y2);
      hit = !(r2x<minX || r1x>maxX || r2y<minY || r1y>maxY); // AABB 교차
    } else if(s.type === 'circle' || s.type === 'arc'){
      // 원: 박스와 원의 경계가 겹치거나 중심이 박스 안
      hit = ib(s.cx,s.cy);
      if(!hit){
        // 박스의 가장 가까운 점이 원 둘레 근방이면 선택
        const nx = Math.max(minX, Math.min(s.cx, maxX));
        const ny = Math.max(minY, Math.min(s.cy, maxY));
        const d = Math.hypot(s.cx-nx, s.cy-ny);
        hit = (d <= s.r + 0.5); // 둘레 근처
      }
    } else if(s.type === 'fill' && Array.isArray(s.vertices)){
      // 채움: 정점 중 하나라도 박스 안, 또는 변이 박스와 교차
      hit = s.vertices.some(v => ib(v.x,v.y));
      if(!hit){
        for(let k=0;k<s.vertices.length && !hit;k++){
          const a=s.vertices[k], b=s.vertices[(k+1)%s.vertices.length];
          hit = segIntersectsBox({x1:a.x,y1:a.y,x2:b.x,y2:b.y}, minX,minY,maxX,maxY);
        }
      }
    } else return;
    if(hit) state.selectedShapes.add(i);
  });
  const n = state.selectedShapes.size;
  const np = state.selectedPenPoints.size;
  setStat('📦 박스 선택: 도형 ' + n + '개' + (np ? ' · 점 ' + np + '개' : '') + (box.addToSel ? ' (추가)' : ''));
}
...
function deleteSelected(){
  if(state.mode === 'sketch'){
    // v9.11: 박스로 선택된 펜점 + 도형을 함께 삭제
    const hasBoxPts = state.selectedPenPoints && state.selectedPenPoints.size > 0;
    const hasShapes = state.selectedShapes && state.selectedShapes.size > 0;
    const hasPen = state.penCur >= 0 && state.penPoints[state.penCur];
    if(!hasShapes && !hasPen && !hasBoxPts){ toast('선택된 도형/점이 없습니다'); return; }
    // 박스로 선택된 점들이 있으면 그 점들 + 선택 도형을 삭제
    if(hasBoxPts){
      pushHistory();
      // 선택 도형 먼저 삭제
      let delShapes = 0;
      if(hasShapes){
        ...
        delShapes = idxs.length;
        state.selectedShapes.clear();
      }
      // 선택된 펜점 삭제 (좌표 수집 후 그 좌표를 끝점으로 하는 선도 정리)
      const selPts = [...state.selectedPenPoints].sort((a,b)=>b-a);
      const tol = 0.05;
      const ptCoords = selPts.map(i => state.penPoints[i]).filter(Boolean).map(p=>({x:p.x,y:p.y}));
      selPts.forEach(i => state.penPoints.splice(i,1));
      // 그 점에 붙어있던 선분도 제거
      const beforeLines = state.shapes.length;
      state.shapes = state.shapes.filter(s => {
        if(s.type !== 'line') return true;
        const onPt = (x,y) => ptCoords.some(c => Math.abs(c.x-x)<tol && Math.abs(c.y-y)<tol);
        return !(onPt(s.x1,s.y1) || onPt(s.x2,s.y2));
      });
      const delLines = beforeLines - state.shapes.length;
      state.selectedPenPoints.clear();
      if(state.penCur >= state.penPoints.length) state.penCur = state.penPoints.length - 1;
      redrawSketch(); updateInfo();
      if(typeof window.sk3UpdateSelProp === 'function') window.sk3UpdateSelProp();
      toast('🗑 점 ' + selPts.length + '개' + (delShapes? ' + 도형 '+delShapes+'개':'') + (delLines? ' + 연결선 '+delLines+'개':'') + ' 삭제');
      return;
    }
    pushHistory();
    if(hasShapes){
...
function initThree(){
  ...
  // v9.17: 저장된 배경색 복원 + 피커 동기화
  try{
    const savedBg = localStorage.getItem('dt3_bg_color');
    const hex = (savedBg && /^#[0-9a-fA-F]{6}$/.test(savedBg)) ? savedBg : '#1a1a1a';
    scene.background = new THREE.Color(hex);
    const pick = document.getElementById('bgColorPick');
...
function clearWorkPlane(){
  ...
  if(axesHelper) axesHelper.visible = state.showAxes; // v9.16: 작업면 해제 시 축 표시 상태 복원
...
function setWorkPlaneFromHit(hit){
  ...
  if(axesHelper) axesHelper.visible = false; // v9.16: 작업면 활성 시 전역 축 숨김 → 평면만 표시
...
function onBgColorChange(hex){
  ...
  if(typeof scene !== 'undefined' && scene) scene.background = new THREE.Color(hex);
  try{ localStorage.setItem('dt3_bg_color', hex); }catch(e){}
}
function resetBgColor(){
  ...
  const pick = document.getElementById('bgColorPick');
  if(pick) pick.value = hex;
  try{ localStorage.removeItem('dt3_bg_color'); }catch(e){}
  toast('🎨 배경색 기본값');
}
function openPosSizeModal(){
  ...
  // v9.14: 곡면 도형이면 면(분할) 수 표시
  const segRow = document.getElementById('psSegRow');
  if(segRow){
    if(isSegmentPart(target)){
      segRow.style.display = '';
      document.getElementById('psSegments').value = getPartSegments(target);
    } else {
...
function applyPosSize(){
  ...
  // v9.14: 면(분할) 수 변경 → geometry 재생성 (크기 역산 전에 수행, 크기는 아래 scale로 보존)
  if(isSegmentPart(target)){
    const inp = document.getElementById('psSegments');
    if(inp && inp.value !== ''){
      const wantSeg = _clampSeg(inp.value);
      if(wantSeg !== getPartSegments(target)){
        rebuildPartSegments(target, wantSeg);
        target.mesh.updateMatrixWorld(true);
      }
    }
  }
...
function saveProject(){
  // v9.18: 2D(스케치)/3D(부품) 구분
  const n2d = (state.shapes || []).length;
  const n3d = (state.parts || []).length;
  let kind = 'empty';
  if(n2d > 0 && n3d > 0) kind = '2D3D';
  else if(n3d > 0) kind = '3D';
  else if(n2d > 0) kind = '2D';
  const data = {
    version: '1.2',
    savedAt: new Date().toISOString(),
    contentKind: kind,                       // v9.18: 2D / 3D / 2D3D
    counts: { shapes2D: n2d, parts3D: n3d },  // v9.18: 내용 개수
    shapes: state.shapes,
  ...
  a.href = url;
  a.download = `tool3_${kind}_${new Date().toISOString().slice(0,10)}_${Date.now()}.json`;
  a.click();
  URL.revokeObjectURL(url);
  toast(`💾 저장 완료 · ${kind} (2D ${n2d} / 3D ${n3d})`);
}
...
function loadProject(e){
  ...
  reader.onload = (ev)=>{
    try{
      ...
      redrawSketch();
      updateInfo();
      // v9.18: 불러온 내용 종류/개수 표시
      const n2d = (data.shapes || []).length;
      const n3d = (data.parts || []).length;
      const kind = data.contentKind ||
        ((n3d > 0 && n2d > 0) ? '2D3D' : n3d > 0 ? '3D' : n2d > 0 ? '2D' : 'empty');
      toast(`📂 불러오기 완료 · ${kind} (2D ${n2d} / 3D ${n3d})`);
    } catch(err){
...
// ===== v9.14: 곡면 기본도형 면(분할) 수 편집 =====
// geom.parameters(THREE 생성 파라미터) 기준으로 면 수만 바꿔 재생성 → 생성 경로 무관
const SEG_KINDS = { cylinder: 1, sphere: 1, cone: 1, torus: 1 };

function _primKind(part){
  let k = part && part.params && part.params.palette;
  if(!k && part && part.type && part.type.indexOf('primitive_') === 0){
    k = part.type.replace('primitive_', '');
  }
  return k || '';
}
function isSegmentPart(part){ return !!SEG_KINDS[_primKind(part)]; }

function _clampSeg(n){
  n = Math.round(Number(n));
  if(!isFinite(n)) n = 32;
  return Math.max(3, Math.min(256, n));
}

function getPartSegments(part){
  const kind = _primKind(part);
  const geom = part && part.mesh && part.mesh.geometry;
  const p = geom && geom.parameters;
  if(p){
    if(kind === 'cylinder' || kind === 'cone') return p.radialSegments;
    if(kind === 'sphere') return p.widthSegments;
    if(kind === 'torus') return p.tubularSegments;
  }
  if(part && part.params && part.params.segments) return part.params.segments;
  return kind === 'torus' ? 48 : 32;
}

// kind/기존 geom 의 치수를 유지한 채 면 수만 바꾼 새 geometry 반환
function rebuildGeomSegments(kind, geom, seg){
  const p = (geom && geom.parameters) || {};
  seg = _clampSeg(seg);
  const TWO = Math.PI * 2;
  if(kind === 'cylinder'){
    return new THREE.CylinderGeometry(
      p.radiusTop != null ? p.radiusTop : 15,
      p.radiusBottom != null ? p.radiusBottom : 15,
      p.height != null ? p.height : 30,
      seg, p.heightSegments || 1, p.openEnded || false,
      p.thetaStart || 0, p.thetaLength != null ? p.thetaLength : TWO);
  }
  if(kind === 'cone'){
    return new THREE.ConeGeometry(
      p.radius != null ? p.radius : 15,
      p.height != null ? p.height : 30,
      seg, p.heightSegments || 1, p.openEnded || false,
      p.thetaStart || 0, p.thetaLength != null ? p.thetaLength : TWO);
  }
  if(kind === 'sphere'){
    const ws = p.widthSegments || 32, hs = p.heightSegments || 24;
    const ratio = ws > 0 ? hs / ws : 0.75;
    const h = Math.max(2, Math.round(seg * ratio));
    return new THREE.SphereGeometry(
      p.radius != null ? p.radius : 15, seg, h,
      p.phiStart || 0, p.phiLength != null ? p.phiLength : TWO,
      p.thetaStart || 0, p.thetaLength != null ? p.thetaLength : Math.PI);
  }
  if(kind === 'torus'){
    const ng = new THREE.TorusGeometry(
      p.radius != null ? p.radius : 20,
      p.tube != null ? p.tube : 5,
      p.radialSegments || 16, seg,
      p.arc != null ? p.arc : TWO);
    ng.rotateX(Math.PI / 2); // 팔레트 도넛과 동일하게 바닥에 눕힘
    return ng;
  }
  return null;
}

// 부품의 geometry 를 새 면 수로 교체 (스케일/위치/회전 유지)
function rebuildPartSegments(part, seg){
  const kind = _primKind(part);
  if(!SEG_KINDS[kind] || !part.mesh || !part.mesh.geometry) return false;
  const ng = rebuildGeomSegments(kind, part.mesh.geometry, seg);
  if(!ng) return false;
  const old = part.mesh.geometry;
  part.mesh.geometry = ng;
  if(old && old.dispose) old.dispose();
  if(!part.params) part.params = {};
  part.params.segments = _clampSeg(seg);
  return true;
}
...
function rebuildPrimitive(pdata){
  ...
  if(!geom) return null;
  // v9.14: 저장된 면 수 복원 (곡면 도형)
  if(p.segments && SEG_KINDS[kind]){
    const ns = rebuildGeomSegments(kind, geom, p.segments);
    if(ns){ if(geom.dispose) geom.dispose(); geom = ns; }
  }
  const mat = makeMaterial(pdata.color, pdata.opacity);
  const mesh = new THREE.Mesh(geom, mat);
...
// v9.10: 격자 입력칸 제거됨 — 0.1mm 고정. (구 핸들러 안전 가드)
(function(){
  const gsEl = document.getElementById('gridSize');
  if(gsEl) gsEl.addEventListener('change', (e)=>{
    state.gridSize = Math.max(0.1, parseFloat(e.target.value) || 0.1);
    redrawSketch();
  });
})();
